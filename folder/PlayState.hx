...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'the-end':
				dialogue = CoolUtil.coolTextFile(Paths.txt('the-end/dialog'));
			case 'finale':
				dialogue = CoolUtil.coolTextFile(Paths.txt('finale/dialog'));
			case 'dababy':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dababy/dadialog'));
		}
		...
		case 'Under':
			{
		                defaultCamZoom = 0.7;
						curStage = 'Under';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Nevada'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
		...
		case 'UnderEvil':
			{
			            defaultCamZoom = 0.7;
						curStage = 'UnderEvil';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Nevada2'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
		...
		case 'Undertroll':
			{
				        defaultCamZoom = 0.7;
						curStage = 'Undertroll';					
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Nevada3'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
		...
		switch (curStage)
		{
            ...
            case 'UnderEvil':
				if(FlxG.save.data.distractions){
				// trailArea.scrollFactor.set();
				var evilTrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.010);
				// evilTrail.changeValuesEnabled(false, false, false, false);
				// evilTrail.changeGraphic()
				add(evilTrail);
				// evilTrail.scrollFactor.set(1.1, 1.1);
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 3 ? "Hell" : storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
		kadeEngineWatermark.setFormat(Paths.font("reap.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		replayTxt.setFormat(Paths.font("reap.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		botPlayState.setFormat(Paths.font("reap.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
